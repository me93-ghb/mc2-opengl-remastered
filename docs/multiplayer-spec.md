...
MP-3 completed 2026-09-14: world updates (mines, tree fires, artillery strikes, building captures, mission-script messages, kill/loss tallies) go out as plain `MpWorldEntry` records in `MCMSG_WorldUpdate`; hold-position ships the client's selection plus range settings and the host patches the live orders; client artillery requests run through the host's `CallArtillery`; resource-point deltas and force-group drops relay (`sendReinforcement` stages 6/7); turrets are indexed into `turretRoster` at mission start and get 10 Hz target updates plus relayed fire; the rematch loop is verified by `run_mp_smoke.py --rematch`.
2026-09-16: VTOL reinforcement drops relay (stage 0 request, host-assigned roster slot, stage 2 creates the vehicle everywhere; `run_mp_smoke.py --orders vtol` buys a minelayer, lays mines and calls artillery, all verified on the client). Drop-outs verified (`--drop client`, `--drop host`): acceptance 3 and 4 hold with 5/10 s peer timeouts (a dead host is noticed in about 15 s). MP-4 default-on flip and MP-5 host:port field are in.
Karnov recovery (stages 3/5) relays as well, verified by `run_mp_smoke.py --orders recover`. Still stubbed: `sendPlayerMoverGroup` (no caller), pilot kill stats, refit hits. Fixed 2026-09-13: pilots did not engage on their own because `pbrain` sets NotTargetingMod to 0 (an enemy that targets nobody rates 0, so a player lance only returns fire); `execSetRealMemory` keeps the orders default in multiplayer, and `run_mp_smoke.py --orders move` proves both lances engage without attack orders.
Client visibility of the enemy lance follows the normal sensor rules (standard sensors have range 1, so contacts appear only in line of sight); `[MP_SENSOR]` on the client shows contacts once the lances close.
